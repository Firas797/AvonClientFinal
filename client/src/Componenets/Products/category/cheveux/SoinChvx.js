...
import SoinChv from './shmpJson/SoinChv';
...
function SoinChvx() {
  ...
  const openModal = (image) => {
    const product = SoinChv.find(item => item.OffersNo === image);
  ...
  const displayedProducts = SoinChv.filter(product => {
    try {
