...
import ApresShmp from './shmpJson/ApresShmp';
...
function AaprShmp() {
  ...
  const openModal = (image) => {
    const product = ApresShmp.find(item => item.OffersNo === image);
  ...
  const displayedProducts = ApresShmp.filter(product => {
    try {
